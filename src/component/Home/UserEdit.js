...
const UserEdit = () => {
    ...
    const { id } = useParams()
    useEffect(() => {
        fetch(`https://still-plateau-84079.herokuapp.com/user/${id}`)
            .then(res => res.json())
    ...
    const onSubmit = data => {
        console.log(data);
        const url = `https://still-plateau-84079.herokuapp.com/user/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
    ...
    return (
        <div style={{ marginTop: '150px' }} >
            ...
            <form className='w-75 h-75 mx-auto  text-start' onSubmit={handleSubmit(onSubmit)} >

                {
                    edit.name && <div class="form-group mb-2">
                        <input  defaultValue={edit.name}  {...register("name")} type="text" class="form-control text-info fw-bold" placeholder=" Name *" />
                    </div>
                }

                {
                    edit.mulisectors && <div class="form-group mb-2">
                        <select
                            required
                            {...register("mulisectors")} class="form-select" size="7" aria-label="size 4 select example">
                            <option className='text-info fw-bold'>{edit.mulisectors}</option>
                ...
                <input className='btn btn-info m-0 text-start px-5 ' type="submit" name="" id="" />
