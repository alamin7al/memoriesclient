...
const Home = () => {
    ...
    useEffect(() => {
        fetch(`https://still-plateau-84079.herokuapp.com/userlist`)
            .then(res => res.json())
            ...
            fetch(`https://still-plateau-84079.herokuapp.com/user/${id}`, {
                ...
                .then(data => {
                    if (data.deletedCount >= 0) {
                        ...
                        const rstrongainignData = userinfo.filter(x => x._id !== id)
                        setUserInfo(rstrongainignData)
                    }
        ...
        <div className="mx-4">
            <div className="row">
                <div className="col-md-5">
                    <div className='  mx-auto my-5 text-start' >
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <input className='btn btn-info m-0 px-5' type="submit" name="" id="" />
                        </form>
                    </div>
                </div>
                                ...
                                userinfo.map((user) =>
                                    <tr key={user._id}>
                                       <td>{user._id}</td>
                                        ...
                                        <td className='d-dlex align-item-center justify-content-evenly'>
                                            <div className="d-flex">

                                                <Link to={`/edit/${user._id}`}>
                                                    <button className='btn btn-info m-0 me-2'>Edit</button>
                                                </Link>
                                                <button onClick={() => handleDeleted(user._id)} className='btn btn-danger m-0 me-2'>Delete</button>
                                            </div>
                                        </td>
                                    </tr>
                                )
